...
n <- 100; t <- rep(c(0, 1), c(n/2, n/2)); x <- c(runif(n/2), runif(n/2)); 
  ##  t is the treatment effect; 50 zero followed by 50 ones.
beta0 <- 0; beta1 <- 2
tau <- 1  ## tau is the treatment effect parameter.
sigma <- .2
y <- beta0 + x * beta1 + t * tau + rnorm(n, sd = sigma)
  ## that last term is effectively the error (epsilon).
plot(x, y, type = "n", frame = FALSE)
  ## type='n' makes the frame of plot only; points and lines added later.
  ## line disregarding treatment
abline(lm(y ~ x), lwd = 2)
  ## means of the two groups (horizontal):
abline(h = mean(y[1 : (n/2)]), lwd = 3)
abline(h = mean(y[(n/2 + 1) : n]), lwd = 3)
  ## model including treatment, and lines for the two groups.
fit <- lm(y ~ x + t)
abline(coef(fit)[1], coef(fit)[2], lwd = 3)
abline(coef(fit)[1] + coef(fit)[3], coef(fit)[2], lwd = 3)
  ## colored points on top of the lines.
