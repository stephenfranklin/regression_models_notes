...
options(width = 100)
require("knitr")
opts_chunk$set(message = F, error = F, warning = F, comment = NA, fig.align = 'center', dpi = 100, tidy = F, cache.path = '.cache/', fig.path = 'fig_03_02/')
...
* Collection of exchangeable binary outcomes for the same covariate data are called binomial outcomes.
    * Heads and tails are exchangeable, for example.
* Binomial Outcomes: Instead of analyzing the individual 0s and 1s, we analyze the count of 0s and 1s out of the number of flips.
...
```{r getRavens, echo=FALSE, eval=FALSE}
dir.create(path = paste0(getwd(),"/files_03_02"))
download.file("http://dl.dropboxusercontent.com/u/7710864/data/ravensData.rda"
              , destfile=paste0(getwd(),"/files_03_02/ravensData.rda"),method="curl")
```
```{r loadRavens,cache=TRUE}
load("./files_03_02/ravensData.rda")
head(ravensData)
```
...
A linear regression model doesn't fit very well to this example.
* If the additive error is continuous, it make the result something other than 0 and 1.
* Otherwise, the error would have to depend on the intercept and slope to result in a 0 or 1.
...
Here we fit the linear regression even though it's not the most easily interpretable way of fitting this example.
...
If there were other variables involved they would be held fixed.
...
Here we have a `manipulate()` which won't work as a pdf.
...
* `family=binomial` indicates a logistic regression.
* The intercept ($\beta_0$, which is -1.6800) is the log odds of the Ravens winning if they score nothing.
    * It makes sense that it's negative because if they score nothing we wouldn't expect them to win.
* If we want the odds of them winning we can take the exponent of the intercept.
    * $\exp (-1.68) = 0.186$
* The slope ($\beta_1$, which is 0.1066) is the increase in the log odds for every point that they score.
    * The exponent of that is the relative increase (the odds ratio) for a one unit increase in score.
    * $\exp (0.1066) = 1.112$
* If we had included other variables, that interpretation would be holding them constant.
