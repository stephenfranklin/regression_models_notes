...
options(width = 100)
opts_chunk$set(message = F, error = F, warning = F, comment = NA, fig.align = 'center', dpi = 100, tidy = F, cache.path = '.cache/', fig.path = 'fig_02_01/')
...
* How can one generalize SLR (simple linear regression) to incoporate lots of regressors for the purpose of prediction?
...
* Least squares (and hence maximum likelihood estimates  (ML) under iid Gaussianity 
of the errors) minimizes
$$
\sum_{i=1}^n \left(Y_i - \sum_{k=1}^p X_{ki} \beta_j\right)^2
$$
* Note, the important linearity is linearity in the coefficients as opposed to the variables.
Thus
$$
...
predictor variables.)
    * In other words, we could have variables such as $X$, $X^2$, $\sin{X}$, or anything.
    
---
## How to get estimates
* The real way requires linear algebra. We'll go over an intuitive development instead. 
* Recall that the LS (Least Squares) estimate for regression through the origin, $E[Y_i]=X_{1i}\beta_1$, was $\sum X_i Y_i / \sum X_i^2$.
...
$$
* Thus, to get the least squares estimates, we need to get these two equations to be zero: 
  1. $\sum_{i=1}^n (Y_i - \hat \beta_1 X_{1i} - \hat \beta_2 X_{2i}) X_{1i} = 0$ 
  2.  $\sum_{i=1}^n (Y_i - \hat \beta_1 X_{1i} - \hat \beta_2 X_{2i}) X_{2i} = 0$
      * And if we have more than two regressors, we simply have more of those equations.
...
* This term $\frac{\sum_j X_{2j}Y_j}{\sum_j X_{2j}^2}$ is the slope estimate from the regression through the origin model of $Y_i = X_{2i} + \beta_2 + \epsilon$.
    * If we were to multiply by $X_{2i}$ we would get the predicted $Y_i$ from the model for $Y_i$ above.
    * So $Y_i$ minus the predicted $Y_i$ is the residual.
* Similarly, in second term $\frac{\sum_j X_{2j}X_{1j}}{\sum_j X_{2j}^2}$ is the residual for the model $X_{1i}=X_{2i} + \gamma + \epsilon_i$ (where $\gamma$ is whatever). 
    * Multiplied by $X_{2i}$ we get the predicted version of $X_{1i}$.
    * And subtracted by the observed $X_{i1}$ we get the residual.

---
## Continued
* By writing that equation as "the $i^{th}$ residual if  we were to have regressed $X_2$ out of $Y$ minus $\hat{\beta}_1$ times the $i^{th}$ residual should we have regressed $X_2$ our of $X_1$ (which in both cases they're regression through the origin)", we get:
$$
0 = \sum_{i=1}^n \left\{ e_{i, Y | X_2} - \hat \beta_1 e_{i, X_1 | X_2} 
\right\} X_{1i} 
$$
where the residual of $a$ given $b$, $e_{i, a | b} = a_i -  \frac{\sum_{j=1}^n a_j b_j }{\sum_{i=1}^n b_j^2} b_i$ is the residual when regressing $b$ from $a$ without an intercept.
* We get the solution
$$
\hat \beta_1 = \frac{\sum_{i=1}^n e_{i, Y | X_2} e_{i, X_1 | X_2}}{\sum_{i=1}^n e_{i, X_1 | X_2} X_1}
$$
* Notice that denominator; we're about to replace it.
...
$$
But the sum of the residuals times the regressor equals zero: $\sum_{i=1}^n e_{i, X_1 | X_2} X_{2i} = 0$.  
So we get that
$$
...
$$
* The denominator has been replaced.
* Think of it this way: there are two vectors: 
    * the response $V_{1i}$ and  
    * the predictor $V_{2i}$.
    * So the equation above is just 
    $$\frac{ \sum V_{1i} V_{2i} }{\sum V_{2i}^2}$$
    * which is the slope estimate from the regression through the origin.
...
* That is, the regression estimate for $\beta_1$ is the regression through the origin estimate having regressed $X_2$ out of both the response and the predictor.
    * It's as if we've removed $X_2$ and are looking at the response $Y$ from $X_1$ alone.
...
y <- x + x2 + x3 + rnorm(n, sd = .1)  ## slopes are all 1, error is .1
## A residual function: the residual of 'a' having regressed out 'b'.
e <- function(a, b) a -  sum( a * b ) / sum( b ^ 2) * b
ey <- e(e(y, x2), e(x3, x2))  ## residual of y w/o x2 and x3.
ex <- e(e(x, x2), e(x3, x2))  ## residual of x w/o x2 and x3
sum(ey * ex) / sum(ex ^ 2)  ## slope of regression through the origin.

## Here's R's method 
## (which is actually a different method which is more efficient).
...
---
## Residuals again, a different method to acheive the same thing:
```{r}
ey <- resid(lm(y ~ x2 + x3 - 1))  ## resid of y w/o x2 and x3
ex <- resid(lm(x ~ x2 + x3 - 1))  ## resid of x w/o x2 and x3
sum(ey * ex) / sum(ex ^ 2)
coef(lm(y ~ x + x2 + x3 - 1)) #the -1 removes the intercept term
```
...
* Some amazing things that you can accomplish with linear models
  * Decompose a signal into its harmonics.
    * The discrete Fourier transform is a linear model where the $X$ terms are trigonometric terms.
