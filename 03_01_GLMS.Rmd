...
options(width = 100)
require('knitr')
opts_chunk$set(message = F, error = F, warning = F, comment = NA, fig.align = 'center', dpi = 100, tidy = F, cache.path = '.cache/', fig.path = 'fig_03_01/')
...
* Linear models are the most useful applied statistical technique. However, they are not without their limitations.
  ...
  * Transformations are often hard to interpret. 
    * There's value in modeling the data on the scale that it was collected,                        
        * or the scale that's of interest.
...
* Define the linear predictor to be $\eta_i = \sum_{k=1}^p X_{ik} \beta_k$.
     * The systematic component $\eta_i$ that relates $i$ parameters that are reduced down to $p$ parameters via the covariance $X_{ik}$.
...
$g$ is the (natural) log odds, referred to as the **logit**.
    * Recall that the *odds* are the probability over 1 minus the probablility: 
    * odds = $\frac{p}{1-p}$
* The logit ('LOW-jit') is the log of the odds.
...
* Assume that $Y_i \sim Poisson(\mu_i)$ so that $E[Y_i] = \mu_i$ where $\mu_i \geq 0$
* Linear predictor $\eta_i = \sum_{k=1}^p X_{ik} \beta_k$
* Link function 
$g(\mu) = \eta = \log(\mu)$
...
* For Bernoulli / binomial case $Var(Y_i) = \mu_i (1 - \mu_i)$
* For the Poisson case $Var(Y_i) = \mu_i$. 
* Remember that $\mu_i$ is a function of the linear predictor which involves the Xs and Betas (plural).
    * In R, the function `glm()` will do this for you. 
...
$$
* These are called 'quasi-likelihood' normal equations which use an extra parameter $\phi$ (phi) which acts to relax the assumptions relating the means and the variances which are very strict assumptions in the normal equation models above.
* The quasi-likelihood equations have all the same properties that normal GLMs do, but they don't correspond to a specific model.
...
* Coefficients are interpretted on the link function scale.
* $\beta_k$ is defined as $g$ of the expected value of a difference in a 1 unit increase of a particular coefficient while the other coefficients are held constant, again, on the link function's scale.
$$
